...
func (qb *QueryBuilder) And(queryFragment string, arg any) *QueryBuilder {
...
func (qb *QueryBuilder) First() *sql.Row {
	// Define o limite como 1, a menos que um limite menor já tenha sido definido
	if qb.limit == 0 || qb.limit > 1 {
		qb.limit = 1
	}
	
	sql, args := qb.buildSelectSQL()
	return qb.repo.db.QueryRowContext(qb.ctx, sql, args...)
}